...
# Carrega chaves locais se existirem (para rodar no PC)
load_dotenv()
...
# Função para conectar ao S3 de forma eficiente
...
# === 1. Carregar dados de Votos de forma Inteligente ===
@st.cache_data
def carregar_dados_s3(nome_arquivo):
    # Definimos apenas as colunas estritamente necessárias para o App poupar RAM
    colunas_para_ler = [
        "NM_UE", "DS_CARGO", "SQ_CANDIDATO", "NM_CANDIDATO", 
        "code_meso", "name_meso", "latitude", "longitude", "QT_VOTOS_NOMINAIS_VALIDOS"
    ]
    
    # Otimização de tipos de dados para reduzir o tamanho na memória RAM
    tipos_dados = {
        "NM_UE": "category",
        "DS_CARGO": "category",
        "SQ_CANDIDATO": "str",
    ...
    with st.spinner("Carregando base de votação do S3..."):
        with abrir_arquivo_s3(nome_arquivo, encoding="utf-8") as f:
            df = pd.read_csv(f, sep=",", usecols=colunas_para_ler, dtype=tipos_dados)
...
# Seleção do SQ_CANDIDATO de forma idêntica ao seu código original
...
df_zona = (
    df_filtro.groupby(["code_meso", "name_meso"])
    .agg({
...
# Proteção para lat/lon vazios caso mude o filtro
...
# === 3. Carregar dados de Despesas de forma Inteligente ===
@st.cache_data
def carregar_despesas_s3(nome_arquivo):
    # Poupar memória lendo apenas SQ_CANDIDATO e as colunas que contenham valores de DESPESA
    with st.spinner("Buscando dados de despesas no S3..."):
        with abrir_arquivo_s3(nome_arquivo, encoding="latin1") as f:
            # Lemos a primeira linha apenas para descobrir os nomes das colunas
            colunas_amostra = pd.read_csv(abrir_arquivo_s3(nome_arquivo, encoding="latin1"), nrows=1)
            colunas_amostra.columns = colunas_amostra.columns.str.strip().str.upper()
            
            colunas_necessarias_desp = [col for col in colunas_amostra.columns if "DESPESA" in col or col == "SQ_CANDIDATO"]
            
        with abrir_arquivo_s3(nome_arquivo, encoding="latin1") as f:
            df = pd.read_csv(f, sep=",", usecols=lambda c: c.strip().upper() in colunas_necessarias_desp)
