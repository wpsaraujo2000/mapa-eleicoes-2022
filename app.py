...
# O Athena é o equivalente exato do BigQuery na AWS.
# Ele vai ler as credenciais automaticamente do seu .streamlit/secrets.toml
conn = st.connection("aws_athena", type="sql")

# Nome do banco e tabela que você vai configurar no AWS Athena
TABELA = "ele2022" 

st.title("🗳️ Mapa de Votação por Mesorregião (2022)")

# === 2. Filtros Dinâmicos (Consultas ultra rápidas via SQL no Athena) ===
@st.cache_data(ttl=3600)
def obter_ufs():
    df_ufs = conn.query(f"SELECT DISTINCT NM_UE FROM {TABELA} ORDER BY NM_UE")
...
def obter_cargos(where_clause):
    df_cargos = conn.query(f"SELECT DISTINCT DS_CARGO FROM {TABELA} WHERE {where_clause} ORDER BY DS_CARGO")
...
# === 3. Consultas Analíticas (A Mágica do SQL, agora na AWS) ===
query_zona = f"""
    SELECT 
        code_meso, 
        name_meso, 
...
# === 4. Processamento Visual e PyDeck (Mantido 100% igual) ===
if not df_zona.empty:
    ...
    st.dataframe(
        df_zona[[
            "code_meso", "name_meso", "VOTOS_CANDIDATO", "VOTOS_TOTAL_MESO", "PERCENTUAL_TOTAL", "PERCENTUAL_MESO"
...
# === 5. Despesas do Candidato (Agora no AWS Athena) ===
st.markdown("### 💸 Despesas do candidato")

# Nome da tabela de despesas no Athena
TABELA_DESPESAS = "cus_voto"
sq_candidato_str = str(sq_candidato_selecionado).strip()

# No Athena, convertemos para VARCHAR em vez de STRING
query_despesas = f"""
    SELECT * 
